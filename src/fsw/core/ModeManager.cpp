    ...
    switch (current_mode_) {
        case MissionMode::SAFE:
            ...
            if (rate_norm < config_.safe_to_nominal_rate_threshold) {
                ...
                common::LogInfo("Detumble complete (rate = {:.4f} rad/s), transitioning to NOMINAL", rate_norm);
            }
            break;
...
void ModeManager::transitionToMode(MissionMode new_mode) {
    ...
    common::LogInfo("Mode transition: {} -> {}", getModeString(previous_mode_), getModeString(current_mode_));
...
bool ModeManager::commandMode(MissionMode new_mode) {
    ...
    common::LogInfo("Manual mode command: {} -> {}", getModeString(current_mode_), getModeString(new_mode));
...
void ModeManager::forceModeChange(MissionMode new_mode, const std::string& reason) {
    if (new_mode == current_mode_) {
        common::LogWarning("FDIR force mode change ignored - already in {} mode: {}", getModeString(new_mode), reason);
        return;
    }

    common::LogWarning("FDIR forcing mode change: {} -> {} - Reason: {}", getModeString(current_mode_),
