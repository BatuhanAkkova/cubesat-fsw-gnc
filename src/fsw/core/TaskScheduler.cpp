...
TaskScheduler::TaskScheduler(double base_dt, bool use_real_time)
    ...
    common::LogInfo("TaskScheduler initialized (dt={:.3f}s, real_time={})", 
                    base_dt_, use_real_time_);
}
...
void TaskScheduler::registerTask(const Task& task) {
    ...
    common::LogInfo("Registered task '{}' at {:.1f}Hz (period={:.3f}s, priority={})", 
                    task.name, 1.0/task.period_sec, task.period_sec, task.priority);
}
...
void TaskScheduler::run(double duration_sec) {
    ...
    while (running_) {
        ...
        if (use_real_time_) {
            ...
            if (sleep_time > 0.0) {
                ...
                common::LogWarning("Task overrun: cycle took {:.3f}ms (target {:.3f}ms)", 
                                   elapsed * 1000.0, base_dt_ * 1000.0);
            }
        }
    ...
    common::LogInfo("TaskScheduler stopped at t={:.2f}s (cycles={}, avg_jitter={:.2f}ms, max_jitter={:.2f}ms, overruns={})",
                    current_time_, stats_.cycles_executed, stats_.avg_jitter_ms, 
                    stats_.max_jitter_ms, stats_.task_overruns);
}
