...
int main() {
    ...
    for (int i = 0; i < 60; i++) {
        ...
        if (i % 10 == 0) {
            ...
            std::cout << std::setw(7) << i << " | " << std::setw(20) << std::fixed << std::setprecision(4) << h_total
                      << " | Saturating\n";
        }
    }
    ...
    std::cout << "\nWheels saturated to " << h_total_saturated << " Nms (" << (h_total_saturated / (3 * 0.05) * 100.0)
              << "% of max)\n\n";
    ...
    for (int i = 0; i < 300; i++) {  // 5 minutes
        ...
        if (i % 30 == 0) {
            ...
            switch (desaturator.getState()) {
                case WheelDesaturation::State::IDLE:
                    state_str = "IDLE";
                    break;
                case WheelDesaturation::State::DESATURATING:
                    state_str = "DESATURATING";
                    break;
                case WheelDesaturation::State::COOLDOWN:
                    state_str = "COOLDOWN";
                    break;
            }

            std::cout << std::setw(7) << (int)sim_time << " | " << std::setw(13) << std::fixed << std::setprecision(4)
                      << h_total << " | " << std::setw(11) << std::fixed << std::setprecision(2)
                      << (b_field.norm() * 1e6) << " | " << std::setw(11) << std::scientific << std::setprecision(2)
                      << dipole_cmd.norm() << " | " << state_str << "\n";
        }
    }
