...
TEST_F(TaskSchedulerTest, SingleTaskExecution) {
    ...
    int count = 0;
    Task task("test_task", [&](double dt) { count++; }, 0.1);
...
TEST_F(TaskSchedulerTest, MultiRateTasks) {
    ...
    // 10Hz task
    Task task_10hz("task_10hz", [&](double dt) { execution_count_10hz++; }, 0.1);

    // 1Hz task
    Task task_1hz("task_1hz", [&](double dt) { execution_count_1hz++; }, 1.0);
...
TEST_F(TaskSchedulerTest, TaskPriority) {
    ...
    Task low_priority("low", [&](double dt) { execution_order.push_back("low"); }, 0.1, 1);

    Task high_priority("high", [&](double dt) { execution_order.push_back("high"); }, 0.1, 10);

    Task medium_priority("medium", [&](double dt) { execution_order.push_back("medium"); }, 0.1, 5);
...
TEST_F(TaskSchedulerTest, TaskException) {
    ...
    int count = 0;
    Task good_task("good", [&](double dt) { count++; }, 0.1);

    Task bad_task("bad", [&](double dt) { throw std::runtime_error("Test exception"); }, 0.1);
...
TEST_F(TaskSchedulerTest, DifferentPeriodTask) {
    ...
    Task task_20hz("20hz", [&](double dt) { count_20hz++; }, 0.05);  // 20Hz

    Task task_5hz("5hz", [&](double dt) { count_5hz++; }, 0.2);  // 5Hz
